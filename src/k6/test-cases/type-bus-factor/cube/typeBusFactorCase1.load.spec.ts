...
import { typeBusFactorCubeAssert } from './assert';
...
export default function () {
  group(
    ...
    () => {
      ...
      const response = http.post(`http://localhost:3004/api/cube/type_bus_factor`, {
        timeRangeName, type, project
      } as any);
      check(response, {
        "Should have code status 200": res => res.status === 200,
        "Should contain data": typeBusFactorCubeAssert
      });
    }
  );
};
